...
export default function UserProfile() {
  ...
  const [
    cambiandoSeguimiento,
    setCambiandoSeguimiento,
  ] = useState(false);
  ...
  const abrirChat =
    async () => {
      ...
      try {
        ...
        const datos =
          await crearChatPrivado(
            persona._id,
            sesion.token,
          );

        navigate(
          `/mensajes?chat=${datos.conversacion._id}`,
        );
      } catch (
        errorSolicitud
      ) {
              ...
              {esMiPerfil ? (
                ...
                <div className="mb-2 flex items-center gap-2">
                  <button
                    ...
                    className={`rounded-2xl px-4 py-2.5 text-sm font-semibold transition active:scale-95 disabled:opacity-60 ${
                      persona.siguiendo
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm font-semibold text-slate-300 transition hover:bg-white/10 active:scale-95 disabled:opacity-60"
                  >
                    {abriendoChat ? (
                      <LoaderCircle
                        size={17}
