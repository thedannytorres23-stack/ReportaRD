...
import { obtenerTotalMensajesNoLeidos } from "../services/chatService";
...
const CONTEXTOS = {
  "/": [
    ...
    [
      "Sigue temas de tu sector",
      "Confirma información útil",
  ...
  "/buscar": [
    ...
    [
      "Usa nombres o usuarios",
      "Revisa el perfil antes de conectar",
  ...
  "/mapa": [
    ...
    [
      "Acerca el mapa para precisar",
      "Filtra por categoría",
      "Confirma reportes que conozcas",
    ],
  ],
  ...
  "/mensajes": [
    ...
    [
      ...
      "Reporta conductas inapropiadas",
    ],
  ],
  ...
  "/en-vivo": [
    ...
    [
      "Describe claramente el contexto",
      "Protege la privacidad de terceros",
      "Usa el directo de forma responsable",
    ],
  ],
  ...
  "/notificaciones": [
    ...
    [
      "Revisa menciones recientes",
      "Distingue alertas importantes",
      "Mantén tus preferencias al día",
    ],
  ],
  ...
  "/perfil": [
    ...
    [
      "Usa información auténtica",
      "Añade una ubicación general",
      "Explica cómo aportas a tu comunidad",
    ],
  ],
};
...
export default function DesktopChrome({
  ...
  const [
    ...
    descripcion,
    recomendaciones,
  ] = obtenerContexto(location.pathname);

  const camposPerfil = [
    perfil.nombre &&
      perfil.nombre !== PERFIL_BASE.nombre,

    perfil.usuario &&
      perfil.usuario !== PERFIL_BASE.usuario,
  ...
  const perfilCompletado = Math.round(
    (
      camposPerfil.filter(Boolean).length /
      camposPerfil.length
    ) * 100,
  );
  ...
  const [
    ...
    setMostrarProgresoPerfil,
  ] = useState(() => {
    if (perfilCompletado < 100) {
      return true;
    }

    return (
      localStorage.getItem(
        "reportard_profile_completed_seen",
      ) !== "true"
    );
  });
  ...
  useEffect(() => {
    ...
    const cargarNoLeidos = async () => {
      try {
        const datos =
          await obtenerTotalMensajesNoLeidos(
            token,
          );
    ...
    const actualizarDesdeLectura = () => {
      cargarNoLeidos();
    };

    const actualizarDesdeMensaje = () => {
      cargarNoLeidos();
    };

    const actualizarDesdeNotificacion = (
      notificacion,
    ) => {
      if (
    ...
    const actualizarAlCambiarVisibilidad =
      () => {
        if (
    ...
    document.addEventListener(
      "visibilitychange",
      actualizarAlCambiarVisibilidad,
    );
  ...
  return (
    ...
    >
      <style>{`
        ...
        .reportard-no-scrollbar {
          scrollbar-width: none;
        }

        .reportard-no-scrollbar::-webkit-scrollbar {
          display: none;
        }

        @keyframes reportard-badge-enter {
          0% {
            opacity: 0;
            transform: scale(.6);
          }
        ...
        .reportard-message-badge {
          animation:
            reportard-badge-enter
            .24s ease-out;
        }
      ...
      <aside
        ...
        className="
          ...
          bg-[#081525]/95
          text-white
          shadow-[0_28px_80px_rgba(0,0,0,.34)]
          backdrop-blur-xl
          lg:left-4 lg:flex
          xl:left-[calc(50%_-_37.5rem)]
        "
      >
        <header
          className="
        ...
        >
          <button
            ...
            className="
              ...
              px-4 py-3
              text-sm font-bold
              shadow-[0_14px_35px_rgba(239,68,68,.18)]
              transition
      ...
      <aside
        ...
        className="
          reportard-no-scrollbar
          fixed bottom-4 top-4
          z-30 hidden
          w-[17.75rem]
          overflow-y-auto
          text-white
          xl:right-[calc(50%_-_37.5rem)]
          xl:block
        "
      >
        <section
          ...
          className="
            ...
            border border-slate-700/60
            bg-[#081525]/95
            shadow-[0_28px_80px_rgba(0,0,0,.28)]
            backdrop-blur-xl
          "
        >
          <div
